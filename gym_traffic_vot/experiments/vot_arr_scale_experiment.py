...
def main(args):
    ...
    for a in np.arange(0.01, 1.1, 0.01):
        for s in [1,2,4,8]:

            arg_parser = run.common_arg_parser()
            args, unknown_args = arg_parser.parse_known_args(args)
            extra_args = run.parse_cmdline_kwargs(unknown_args)
            save_path = '~/projects/traffic-gym-vot/models/vot_{}_{}_deepq_'.format(a, s)
            res_path =  '~/projects/traffic-gym-vot/resutls/vot_{}_{}_deepq_'.format(a, s)
            ...
            while True and len(episode_rewards) < 1000:
                ...
                if done:
                    ...
                    f.write('episode_rew={}'.format(episode_rew), f)
                    episode_rew = 0
                    if 0 == len(episode_rewards) % 10:
                        ...
                        f.write('Avg. reward of last {} episodes is [{:.2f}-{:.2f}]'.format(n, m - h, m + h))
                        if h < abs(m * 0.01):
                            break
                    obs = env.reset()
